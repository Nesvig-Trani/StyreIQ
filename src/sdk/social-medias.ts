import { createSocialMediaFormSchema, SocialMediaStatusEnum } from '@/social-medias'
import { z } from 'zod'
 ...
 * @returns
 */
export const changeStatusSocialMedia = async (id: number, newStatus: SocialMediaStatusEnum) => {
  const response = await fetch(`${env.NEXT_PUBLIC_BASE_URL}/api/social-medias/${id}`, {
    method: 'PATCH',
    headers: JSON_HEADERS,
    credentials: 'include',
    body: JSON.stringify({
